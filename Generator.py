...
def load_patch(x_start_val_lvl3, y_start_val_lvl3, filename, level, patch_size):
    ...
    patch = patch_access.getPatchAsImage(level, int(x_start_val_lvl3[0]), int(y_start_val_lvl3[0]), patch_size, patch_size, False)
...
class CustomDataGenerator(tf.keras.utils.Sequence):
    ...
    def __getitem__(self, idx):
        ...
        for i, index in enumerate(current_batch_indices):
            ...
            x_start, y_start, filename, _ = position[1], position[2], position[0], position[3]
